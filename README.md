...
#### 5-4 연관관계의 주인
###### 정리
    테이블은 외래 키 하나로 연관관계를 관리하는데
    양방향 연관관계가 되려면 단방향 연관관계가 두개여야 한다.
    엔티티를 양방향 연관관계로 설정하면 객체의 참조는 둘인데 외래 키는 하나다.
    따라서 둘 사이에 차이가 발생한다.
    이런 차이로 두 객체 연관관계 중 하나를 정해서
    테이블의 외래키를 관리해야 하는데 이것을 연관관계의 주인이라 한다.

    연관관계의 주인만이 데이터베이스 연관관계와 매핑되고
    외래 키를 관리 할 수 있다 (등록, 수정, 삭제)
    주인이 아닌 쪽은 읽기만 할 수 있다.
    
    주인은 mappedBy 속성을 사용하지 않고, 
    주인이 아닌 경우 mappedBy 속성을 통해 주인을 지정한다.
    
    연관관계의 주인은 테이블에 외래 키가 있는 곳으로 정하면 된다.
    
#### 5-5 양방향 연관관계 저장
###### 정리
    양방향 연관관계는 연관관계의 주인이 외래 키를 고나리한다.
    따라서 주인이 아닌 방향은 값을 설정하지 않아도 
    데이터베이스에 외래 키 값이 정상 입력된다.

#### 5-6 양방향 연관관계의 주의점
###### 정리
    양방향 연관관계를 설정하고 가장 흔히 하는 실수는 
    연관관계의 주인에는 값을 입력하지 않고
    주인이 아닌 곳에만 값을 입력 하는 것
    
    순수한 객체를 고려하려면 양쪽 방향에 모두 값을 입력해주는 것이 가장 안전
    
    연관관계를 변경할 때는 기존 연관관계를 삭제하는 코드를 추가
    ORM 에서는 관계가 제거되지 않아도 결국 주인이 아니기 때문에 외래키를 변경하는데는 문제가 없다.
    그러나 고나계를 변경하고 영속성 컨텍스트가 살아있을 때에는
    기존 연관관계를 호출하면 객체가 반환된다. 
    그러므로 연관관계를 제거하는 것이 가장 안전
...
###### 정리
    ...
    항상 서로 참조하게 하려면 연관관계 편의 메서도를 작성하는것이 좋으며
    이 메서드를 양쪽 다 작성할 시에 무한루프에 빠지므로 주의해야 한다.

#### 6-2 일대다
###### 정리
    일대다 관계에서는 외래키가 항상 다쪽 테이블에 있을수 밖에 없으므로, 
    일대다 단방향 관계는 반대쪽 테이블에 있는 외래키를 관리한다.
    일대다 단방향 관계를 매핑할 때는 @JoinColumn 을 명시해야 한다.
    
    반대쪽 테이블에 있는 외래키를 관리하기 위해 연관관계 처리를 해줘야 한다.
    ( 해당 객체를 수정하더라도 그 외래키를 가지고 있는 객체도 수정해줘야 한다는 뜻 )
    
    일대다 양방향 매핑은 일대다 단방향 매핑에 다대일 단방향 매핑을 읽기 전용으로 추가하여 매핑할 수 있다.
    따라서 일대다 단방향 매핑의 단점이 그대로 있기 때문에 다대일 양방향 매핑을 이용하는 것이 좋다.

#### 6-3 일대일
###### 정리
    일대일 관계에서는 반대쪽도 일대일 관계기 때문에,
    어느 테이블이 외래키를 가질것인지가 중요하다.
    
    주 테이블에서 외래키를 가지면 외래키를 객체 참조와 비슷하게 사용할 수 있어서
    주 테이블만 확인해도 대상 테이블과의 연관관계를 알수 있다.

    대상 테이블에서 외래키를 가지면 일대일에서 일대다로 테이블을 변경할때
    그 구조를 그대로 사용할 수 있다.
...
###### 정리
    ...
    단방향 : @JoinTable 과 @ManyToMany 로 매핑
    양방향 : @ManyToMany 사용 mappedBy 를 통해 주인을 지정
    연결 엔티티 사용 : 복합 기본 키 사용
        복합 기본 키 사용은 식별자 클래스르 만들어야 하고,
        Serializable 구현, equals 와 hashCode 메서드 구현, 기본 생성자 가 있어야 한다.
    새로운 기본키 사용 : 데이터베이스에서 자동으로 생성해주는 대리 키 값을 사용

#### 7-1 상속 관계 매핑
###### 정리
    관계형 데이터베이스에는 상속이라는 개념을 슈퍼타입 서브타입 관계라는 모델링 기법으로 대신 표현한다.
    - 조인 전략 : 엔티티 각각을 모두 테이블로 만들고 자식 테이블이 부모 테이블의 기본 키를 받아서
                기본 키 + 외래 키로 사용하는 전략
                테이블에는 타입의 개념이 없기 때문에 타입을 구분하는 컬럼을 추가
                장점 : 테이블 정규화, 외래 키 참조 무결성 제약조건 활용, 저장공간 효율적 사용
                단점 : 조인으로 성능 저하, 쿼리 복잡, 데이터 등록할 때 INSERT 가 두 번 실행
                
    - 단일 테이블 전략 : 테이블 하나만 사용하고, 구분 컬럼으로 어떤 자식 데이터가 저장되었는지 구분
                       장점 : 조인이 필요 없어 조회 성능이 빠름, 쿼리 단순
                       단점 : 자식 엔티티 컬럼 모두 null 허용, 테이블이 커질수 있음
    - 구현 클래스마다 테이블 전략 : 엔티티 각각을 모두 테이블로 만들고 자식 테이블 각각에 필요한 컬럼이 모두 있다.
                                장점 : 서브 타입을 구분해서 처리할 때 효과적
                                단점 : 여러 자식 테이블을 함께 조회할 때 성능이 느림, 통합 쿼리가 어려움
                                 
#### 7-2 @MappedSuperclass
###### 정리
    부모 클래스는 테이블과 매핑하지 않고 부모 클래스를 상속받는 자식 클래스에서 매핑정보만 제공하고 싶을 때 @MappedSuperclass 사용
    물려받은 부모의 매핑 정보를 재정의 하려면 @AttributeOverride, 연관관계를 재정의하려면 @AssociationOverride 사용
    - @MappedSuperclass 로 지정한 클래스는 엔티티가 아니므로 em.find() 나 JPQL 에서 사용할 수 없다.
    - 추상클래스로 만들기를 권장

#### 7-3 복합 키와 식별 관계 매핑
###### 정리
    식별 관계 : 부모 테이블의 기본 키를 내려 받아서 자식 테이블의 기본 키 + 외래 키로 사용하는 관계
    비식별 관계 : 부모 테이블의 기본 키를 받아서 자식 테이블의 외래 키로만 사용
        - 필수적 비식별 관계 : 외래 키에 Null 을 허용하지 않는다.
        - 선택적 비식별 관계 : 외래 키에 Null 을 허용한다.
    
    JPA 는 복합 키를 지원하기 위해 데이터베이스에 가까운 @IdClass 와 객체지향에 가까운 @EmbeddedId 를 지원
        - Serializable 인터페이스 구현
        - equals, hashCode 구현
        - 기본 생성자
        - public 클래스
    @IdClass : 식별자 클래스의 속성명과 엔티티에서 사용하는 식별자의 속성명이 같아야 한다.
    @EmbeddedId : @Embaddable 어노테이션을 붙여줘야 한다.
    
    영속성 컨텍스트는 엔티티의 식별자를 키로 사용해서 엔티티를 관리한다.
    그리고 식별자를 비교할 때 equals() 와 hashCode() 를 사용한다.
    따라서 식별자 객체의 동등성이 지켜지지 않으면 예상과 다른 결과가 나올 수 있다.
    
    식별 관계과 비식별 관계
    
    - 식별 관계는 부모 테이블의 기본 키를 자식 테이블로 전파하면서 기본 키 컬럼이 점점 늘어난다.
    - 식별 관계는 2개 이상의 컬럼을 합해서 복합 기본 키를 만들어야 하는 경우가 많다.
    - 식별 관계는 기본 키로 비즈니스 의미가 있는 자연 키 컬럼을 조합하는 경우가 많다.
    
    이러한 단점들 덕분에 대리 키를 사용하는 비식별 관계가 자주 쓰임
    
    식별 관계의 장점은 하위 테이블이 상위 테이블의 키를 가지고 있으므로 특정 상황에 조회 시 편리하다.
